# Pokerole 3.0 Sheet v3.6.1 (Standalone PWA & Owlbear Rodeo Extension)
...
- **Unified Lookup Tool (Pokémon & Moves):** Built-in multi-factor search engine powered by a pre-compiled search index. Filter across all 1,200+ Pokémon by primary and secondary typing (with "Either Type" or "Exact Dual Match"), abilities and slot, move learnsets with rank learned (Starter through Master), Good Starters, and Legendary/Mythical status. Seamlessly switch to the **Moves** tab to search moves by category, power, accuracy, rank, and effects.
- **Interactive Move & Species Cross-Referencing:** Click any move in a Pokémon's learnset to jump directly to its complete move mechanics card, or click any move in Move Lookup to view an expandable reverse-lookup of every Pokémon that learns it (and at what ranks) with 1-click navigation back to the Pokémon profile!
...
`https://lordcongra.github.io/poke-e-role/manifest.json?v=3.6.1`
