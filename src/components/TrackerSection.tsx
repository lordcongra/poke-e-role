...
export function TrackerSection() {
    ...
    const handleEvadeRoll = () => {
        ...
        const dexTotal = Math.max(
            1,
            s.stats[CombatStat.DEX].base +
                s.stats[CombatStat.DEX].rank +
                s.stats[CombatStat.DEX].buff -
                s.stats[CombatStat.DEX].debuff +
                (itemBuffs.stats.dex || 0)
        );
        const evadeTotal =
            s.skills[Skill.EVASION].base + s.skills[Skill.EVASION].buff + (itemBuffs.skills.evasion || 0);
    ...
    const rollManeuver = () => {
        ...
        const getSoc = (stat: 'cle') =>
            Math.max(
                1,
                s.socials[stat].base +
                    s.socials[stat].rank +
                    s.socials[stat].buff -
                    s.socials[stat].debuff +
                    (itemBuffs.stats[stat] || 0)
            );

        if (maneuver === 'ambush')
            rollGeneric('Ambush', getStat(CombatStat.DEX) + getSkill(Skill.STEALTH), 'dex', false, false, true);
        else if (maneuver === 'cover')
            rollGeneric('Cover an Ally', 3 + getStat(CombatStat.INS), 'will', false, false, true);
        else if (maneuver === 'grapple')
            rollGeneric('Grapple', getStat(CombatStat.STR) + getSkill(Skill.BRAWL), 'str', false, false, true);
        else if (maneuver === 'run')
            rollGeneric('Run Away', getStat(CombatStat.DEX) + getSkill(Skill.ATHLETIC), 'dex', false, false, true);
        else if (maneuver === 'stabilize')
            rollGeneric('Stabilize Ally', getSoc('cle') + getSkill(Skill.MEDICINE), 'cle', false, false, true);
        else if (maneuver === 'struggle')
            rollGeneric(
                'Struggle (Accuracy)',
                getStat(CombatStat.DEX) + getSkill(Skill.BRAWL),
                'dex',
                false,
                false,
                true
    ...
    return (
        <div className="tracker-section" style={{ marginBottom: '10px' }}>
            ...
            >
                <button
                    type="button"
                    className={`collapse-btn panel-toggle-btn ${isCollapsed ? 'is-collapsed' : ''}`}
            ...
            {!isCollapsed && (
                <div className="panel-content-wrapper">
                    ...
                    >
                        <div style={{ display: 'flex', alignItems: 'center', gap: '2px' }}>
                            ...
                            <NumberSpinner
                                ...
                                onChange={(v) => updateTracker('actions', Math.max(0, Math.min(5, v)))}
                                min={0}
                                max={5}
                            />
                        </div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'nowrap' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                                ...
                                <input
                                    ...
                                    onChange={(e) => updateTracker('evade', e.target.checked)}
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                                ...
                                <input
                                    ...
                                    onChange={(e) => updateTracker('clash', e.target.checked)}
                                    className="sheet-save"
                                    style={{ cursor: 'pointer', transform: 'scale(1.1)' }}
                                />
                            </div>
                    ...
                    >
                        <span style={{ fontWeight: 'bold' }}>Take your Chances</span>
                        <TooltipIcon
                            onClick={() =>
                                setTooltipInfo({
                    ...
                    >
                        ...
                        <button
                            ...
                            className="action-button action-button--dark"
                            style={{ flex: '0 0 32px', padding: '2px 0', textAlign: 'center' }}
                        >
                            🎲
                        </button>
                        <button
                ...
                >
                    ...
                    >
                        ...
                        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '15px' }}>
                            <NumberSpinner
                                value={chancesToRoll}
                                onChange={(v) => setChancesToRoll(Math.max(1, Math.min(trackers.chances, v)))}
                                min={1}
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px' }}>
                            <button
                                type="button"
                                onClick={() => setChancesModalOpen(false)}
                                className="action-button action-button--dark"
            ...
            {showClashModal && (
                ...
                >
                    ...
                    >
                        ...
                        >
                            Which attribute are you using to Clash?
                        </p>
                    ...
                    >
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px' }}>
                            ...
                            <button
                                ...
                                style={{ background: '#2E7D32', color: 'white', flex: 1, padding: '6px' }}
                            >
                                🏕️ Rest
                            </button>
                        </div>
