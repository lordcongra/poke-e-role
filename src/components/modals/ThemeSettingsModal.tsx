...
export function ThemeSettingsModal({ onClose }: ThemeSettingsModalProps) {
    ...
    // Read the global override state if it exists
    useEffect(() => {
        try {
    ...
    const handleSave = () => {
        if (applyGlobally) {
            // Save to browser, wipe specific token override so it doesn't conflict
            try {
            ...
            // Save to specific token, remove global override if it was active
            if (activeTokenId) {
    ...
    const handleClear = () => {
        // Wipe everything to return to normal type-based coloring
        if (activeTokenId) {
            setIdentity('themePrimaryOverride', '');
            setIdentity('themeSecondaryOverride', '');
        }
        ...
        <div className="theme-modal__overlay">
            <div className="theme-modal__content">
                ...
                <p className="theme-modal__desc">
                    Override the dynamic typing colors with your own custom Hex codes. 
                    Leave the Secondary Color blank to auto-generate a matching accent!
                </p>
                ...
                <label className="theme-modal__checkbox-container">
                    ...
                    <div className="theme-modal__checkbox-text">
                        ...
                        Applies these colors to ALL sheets on your screen. Uncheck to apply to this specific character only.
                    </div>
                </label>

                <div className="theme-modal__actions">
                    <button type="button" className="action-button action-button--dark theme-modal__btn" onClick={onClose}>
                        <X size={16} /> Cancel
                    </button>
                    <button type="button" className="action-button action-button--red theme-modal__btn" onClick={handleClear}>
                        <Trash2 size={16} /> Clear
                    </button>
                    <button type="button" className="action-button action-button--theme theme-modal__btn" onClick={handleSave}>
                        <Save size={16} /> Save
                    </button>
                </div>
            </div>
