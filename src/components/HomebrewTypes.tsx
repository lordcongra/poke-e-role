...
export function HomebrewTypes() {
    ...
    const loadForEditing = (t: CustomType) => {
        ...
        setName(t.name);
        setColor(t.color);
        setGmOnly(t.gmOnly || false);
        setWeaknesses(t.weaknesses || []);
        setResistances(t.resistances || []);
        setImmunities(t.immunities || []);
        setSEAgainst(t.seAgainst || []);
        setNVEAgainst(t.nveAgainst || []);
        setNoEffectAgainst(t.noEffectAgainst || []);
    };
    ...
    const renderPills = (arr: string[], setter: (v: string[]) => void) => (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '4px' }}>
            {arr.map((val) => (
                <span
                    key={val}
                    onClick={() => canEdit && removeFromArray(val, arr, setter)}
                    style={{
                ...
                <>
                    ...
                    >
                        ...
                        <div style={{ display: 'flex', gap: '10px' }}>
                            <div style={{ flex: 1 }}>
                                <h4 style={{ fontSize: '0.8rem', margin: '0 0 4px 0', color: 'var(--primary)' }}>
                                    Defensive
                                </h4>
                                <button
                                ...
                                <button
                                    ...
                                    className="action-button action-button--dark"
                                    style={{ width: '100%', fontSize: '0.7rem', padding: '4px', marginTop: '6px' }}
                                >
                                    + Resist (0.5x)
                                </button>
                    ...
                    <div style={{ display: 'flex', gap: '8px', marginBottom: '15px' }}>
                        ...
                        >
                            {editingOriginalName ? '💾 Save Type' : '+ Add Type'}
                        </button>
                    </div>
                </>
            )}
            ...
            >
                {visibleTypes.length === 0 ? (
                    ...
                    visibleTypes.map((t) => (
                        ...
                        >
                            <span
                                onClick={() => canEdit && loadForEditing(t)}
                                style={{
                                    background: t.color,
                                    color: 'white',
                                    padding: '2px 8px',
