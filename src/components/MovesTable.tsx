...
export function MovesTable() {
    ...
    const maxMoves =
        3 + Math.max(1, insStat.base + insStat.rank + insStat.buff - insStat.debuff + (itemBuffs.stats.ins || 0));
    ...
    const handleTargetClick = (move: MoveData) => {
        ...
        if (setDmgMatch || moveDesc.includes('set damage')) {
            ...
            rollDicePlus(
                `0d6+${dmgVal}`,
                `💥 ${nickname} used ${move.name || 'a Move'}! (Deals exactly ${dmgVal} Set Damage, ignores defenses)`
            );
            return;
        }
    ...
    return (
        <div className="sheet-panel">
            <datalist id="move-list">
                {[...moveList, ...roomCustomMoves.filter((m) => role === 'GM' || !m.gmOnly).map((m) => m.name)].map(
                    (m) => (
            ...
            <div className="sheet-panel__header">
                ...
                >
                        ...
                        <TooltipIcon
                            onClick={() =>
                                setTooltipInfo({
                                    ...
                                    desc: 'Adds or removes bonus dice to all Accuracy rolls.'
                                })
                            }
                        />
                    ...
                    <span style={{ display: 'flex', alignItems: 'center', gap: '2px', whiteSpace: 'nowrap' }}>
                        ...
                        <TooltipIcon
                            onClick={() =>
                                setTooltipInfo({
                                    title: 'Global Damage',
                                    desc: 'Adds or removes bonus dice to all Damage rolls.'
                                })
                            }
                        />
                        ...
                        <TooltipIcon
                            onClick={() =>
                                setTooltipInfo({
                                    ...
                                    desc: 'Flat Bonus/Penalty to final Successes (e.g., Low Accuracy = -1).'
                                })
                            }
                        />
                    ...
                    <span style={{ display: 'flex', alignItems: 'center', gap: '2px', whiteSpace: 'nowrap' }}>
                        ...
                        <TooltipIcon
                            onClick={() =>
                                setTooltipInfo({ title: 'Global Chance', desc: 'Adds bonus dice to all Chance rolls.' })
                            }
                        />
                        :{' '}
                        <NumberSpinner
                            value={trackers.globalChance}
                            onChange={(v) => updateTracker('globalChance', v)}
                            min={0}
                        />
                        <button
                            type="button"
                        ...
                        >
                            ...
                            <TooltipIcon
                                onClick={() =>
                                    setTooltipInfo({
                                        title: 'Ignored Pain',
                                        desc: "Ignored Pain Penalties (Resets per Scene). Use the 'Ign.Pain(-1W)' button in the Tracker to increase this."
                                    })
                                }
                            />
                            :{' '}
                            <NumberSpinner
                                ...
                                onChange={(v) => updateTracker('ignoredPain', v)}
                                min={0}
                            />
                        </span>
                    )}
                ...
                <button
                    type="button"
                    className="mobile-only-flex action-button action-button--dark"
                ...
                <div className="panel-content-wrapper">
                    <div className="desktop-only-flex table-responsive-wrapper">
                        <table className="data-table" style={{ textAlign: 'left' }}>
                            <thead>
                                <tr>
                                    <th style={{ width: '25px', textAlign: 'center' }} title="Used this round?">
                                        ✔
                                    </th>
                    ...
                    {learnset.length > 0 && (
                        <div style={{ marginTop: '8px' }}>
                            <button
                                ...
                                className="action-button action-button--dark"
                                style={{ width: '100%', fontSize: '0.8rem', padding: '4px', margin: 0 }}
                            >
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '15px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                <NumberSpinner
                                    ...
                                    onChange={(v) => updateTracker('globalAcc', v)}
                                />
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span
                                ...
                                <NumberSpinner
                                    ...
                                    onChange={(v) => updateTracker('globalDmg', v)}
                                />
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span
                                ...
                                <NumberSpinner
                                    ...
                                    onChange={(v) => updateTracker('globalSucc', v)}
                                />
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span
