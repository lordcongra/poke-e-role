...
function AbilityCard({ ability, onRemove }: { ability: CustomAbility, onRemove: () => void }) {
        ...
        <div style={{ background: 'var(--panel-alt)', border: '1px solid var(--border)', borderRadius: '6px', padding: '10px', display: 'flex', flexDirection: 'column', gap: '8px', flexShrink: 0 }}>
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                ...
                <input 
                    ...
                    onBlur={() => localName !== ability.name && updateCustomAbility(ability.id, 'name', localName)}
                ...
                />
                <button onClick={() => setShowTagBuilder(true)} className="action-button action-button--dark" style={{ padding: '6px 12px' }}>🏷️ Tags</button>
            ...
            {!isCollapsed && (
                <>
                    <textarea 
                        ...
                        onBlur={() => localDesc !== ability.description && updateCustomAbility(ability.id, 'description', localDesc)}
                    ...
                    <textarea 
                        ...
                        onBlur={() => localEffect !== ability.effect && updateCustomAbility(ability.id, 'effect', localEffect)}
            ...
            {showTagBuilder && <TagBuilderModal targetId={ability.id} targetType="ability" onClose={() => setShowTagBuilder(false)} />}
        </div>
    );
}
...
export function HomebrewAbilities() {
    ...
    const handleExport = () => {
        const dataStr = JSON.stringify(roomCustomAbilities, null, 2);
    ...
    const filteredAbilities = roomCustomAbilities.filter(a => a.name.toLowerCase().includes(search.toLowerCase()));
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', height: '100%' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                ...
                <button onClick={() => { setSearch(''); addCustomAbility(); }} className="action-button action-button--dark" style={{ padding: '8px', background: '#8E24AA', borderColor: '#8E24AA', whiteSpace: 'nowrap' }}>+ Create New</button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', overflowY: 'auto', flex: 1, paddingRight: '4px', overscrollBehavior: 'contain' }}>
                {filteredAbilities.length === 0 ? (
                    <div style={{ textAlign: 'center', fontStyle: 'italic', color: 'var(--text-muted)', fontSize: '0.9rem', padding: '20px' }}>
                        {roomCustomAbilities.length === 0 ? "No custom abilities yet." : "No abilities match your search."}
                    </div>
                ) : (
                    filteredAbilities.map(ability => (
                        <AbilityCard key={ability.id} ability={ability} onRemove={() => removeCustomAbility(ability.id)} />
                    ))
                )}
            </div>
