...
export async function rollStatus(status: StatusItem, state: CharacterState) {
    ...
    await rollDicePlus(
        `${pool}d6>3${mathMod}`,
        `🩹 ${nickname} rolled ${status.name} Recovery!${tagStr}`,
        'status',
        status.id
    );
}
...
export async function rollAccuracy(move: MoveData, state: CharacterState) {
    ...
    await rollDicePlus(
        `${Math.max(1, dicePool)}d6>3${mathMod}`,
...
export async function executeDamageRoll(
    ...
    if (
        moveDesc.includes('recoil') ||
        itemBuffs.dmgItemNames.some((i) => i.toLowerCase().includes('life orb') || i.toLowerCase().includes('recoil'))
    ) {
