...
export async function generateBuild(config: GeneratorConfig, state: CharacterState): Promise<TempBuild | null> {
    ...
    const extractMoves = (moveObj: unknown, ignoreRank = false) => {
        if (Array.isArray(moveObj)) {
            moveObj.forEach((m: unknown) => {
                ...
                const mRank =
                    typeof m === 'object'
                        ? String(mRec.Learned || mRec.Learn || mRec.Level || mRec.Rank || 'Other')
                        : 'Other';
                if (mName && MOVES_URLS[mName.toLowerCase()] && (ignoreRank || isMoveAllowed(mRank)))
                    legalMoveNames.push(mName);
            });
                    ...
                    mList.forEach((m: unknown) => {
                        const mName =
                            typeof m === 'string'
                                ? m
                                : String(
                                      (m as Record<string, unknown>).Name || (m as Record<string, unknown>).Move || ''
                                  );
    ...
    for (const mName of uniqueMoveNames) {
        ...
        if (data) {
            ...
            fetchedMoves.push({
                ...
                attr:
                    normalizeStat(ATTRIBUTE_MAPPING[String(data.Accuracy1 || '')] || String(data.Accuracy1 || '')) ||
                    'str',
    ...
    if (config.buildType === 'wild') {
        ...
        while (attrPoints > 0 && attrsToAssign.length > 0) {
            ...
            if (currentRank + base < attrLimits[randomAttr]) {
                ...
                attrsToAssign.splice(attrsToAssign.indexOf(randomAttr), 1);
            }
        }
        ...
        while (socPoints > 0 && socsToAssign.length > 0) {
            ...
            if (currentRank + base < 5) {
                ...
                socsToAssign.splice(socsToAssign.indexOf(randomSoc), 1);
            }
        }
        ...
        let skillsToAssign: string[] = config.includePmd
            ? [...ALL_SKILLS]
            : ALL_SKILLS.filter((s) => !pmdSkillsList.includes(s));
        ...
        while (skillPoints > 0 && skillsToAssign.length > 0) {
            ...
            if (genSkill[randomSk] < maxSkillRank) {
                ...
                skillsToAssign.splice(skillsToAssign.indexOf(randomSk), 1);
            }
        }
        ...
        while (socPoints > 0 && dumpSocs.length > 0) {
            ...
            if (genSoc[randomSoc] + base < 5) {
                genSoc[randomSoc]++;
                socPoints--;
            } else dumpSocs.splice(dumpSocs.indexOf(randomSoc), 1);
        }

        const pmdSkillsList = ['crafts', 'lore', 'medicine', 'magic'];
        const validSkills: string[] = [...ALL_SKILLS, ...(config.includeCustom ? customSkillsList : [])].filter(
            (s) => config.includePmd || !pmdSkillsList.includes(s)
        );
        ...
        while (skillPoints > 0 && validSkills.length > 0) {
            ...
            if (genSkill[randomSk] < maxSkillRank) {
                genSkill[randomSk]++;
                skillPoints--;
            } else validSkills.splice(validSkills.indexOf(randomSk), 1);
        }
        ...
        draftedMoves.slice(0, 2).forEach((m) => {
            if (m.attr) {
                if (COMBAT_STATS.includes(m.attr)) coreAttrs.add(m.attr);
                else coreSocs.add(m.attr);
            }
            if (m.dmgStat) {
                if (COMBAT_STATS.includes(m.dmgStat)) coreAttrs.add(m.dmgStat);
                else coreSocs.add(m.dmgStat);
            }
        ...
        for (const attr of coreAttrs) {
            ...
            while (attrPoints > 0 && genAttr[attr] < targetRank) {
                genAttr[attr]++;
                attrPoints--;
            }
        }
        for (const soc of coreSocs) {
            ...
            while (socPoints > 0 && genSoc[soc] < targetRank) {
                genSoc[soc]++;
                socPoints--;
            }
        }
        ...
        for (const sk of coreSkills) {
            while (skillPoints > 0 && genSkill[sk] < targetSkillBoost) {
                genSkill[sk]++;
                skillPoints--;
            }
        }
        ...
        while (attrPoints > 0 && remainingAttrs.length > 0) {
            ...
            if (genAttr[rAttr] + state.stats[rAttr as CombatStat].base < attrLimits[rAttr]) {
                genAttr[rAttr]++;
                attrPoints--;
        ...
        while (socPoints > 0 && remainingSocs.length > 0) {
            ...
            if (genSoc[rSoc] + state.socials[rSoc as SocialStat].base < 5) {
                genSoc[rSoc]++;
                socPoints--;
    ...
    return {
        ...
        moves: draftedMoves,
        maxMoves: baseIns + genAttr['ins'] + 3,
