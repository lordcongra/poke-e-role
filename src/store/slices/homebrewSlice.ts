...
export const createHomebrewSlice: StateCreator<CharacterState, [], [], HomebrewSlice> = (set, get) => ({
    ...
    overwriteAllHomebrewData: async (types, abs, moves, mons, items) => {
        set({
            ...
            roomCustomAbilities: abs,
            roomCustomMoves: moves,
            roomCustomPokemon: mons,
    ...
    mergeCustomAbilityData: (abs) => {
        ...
        abs.forEach((a) => {
            ...
            if (idx !== -1) merged[idx] = { ...a, id: merged[idx].id };
            else merged.push({ ...a, id: crypto.randomUUID() });
        });
    ...
    mergeCustomMoveData: (moves) => {
        ...
        moves.forEach((m) => {
            ...
            if (idx !== -1) merged[idx] = { ...m, id: merged[idx].id };
            else merged.push({ ...m, id: crypto.randomUUID() });
        });
    ...
    mergeCustomPokemonData: (mons) => {
        ...
        mons.forEach((p) => {
            ...
            if (idx !== -1) merged[idx] = { ...p, id: merged[idx].id };
            else merged.push({ ...p, id: crypto.randomUUID() });
        });
    ...
    mergeCustomItemData: (items) => {
        ...
        items.forEach((i) => {
            ...
            if (idx !== -1) merged[idx] = { ...i, id: merged[idx].id };
            else merged.push({ ...i, id: crypto.randomUUID() });
        });
    ...
    mergeAllHomebrewData: async (types, abs, moves, mons, items) => {
        ...
        types.forEach((t) => {
            ...
            if (idx !== -1) mergedTypes[idx] = t;
            else mergedTypes.push(t);
        });
        ...
        abs.forEach((a) => {
            ...
            if (idx !== -1) mergedAbs[idx] = { ...a, id: mergedAbs[idx].id };
            else mergedAbs.push({ ...a, id: crypto.randomUUID() });
        });
        ...
        moves.forEach((m) => {
            ...
            if (idx !== -1) mergedMoves[idx] = { ...m, id: mergedMoves[idx].id };
            else mergedMoves.push({ ...m, id: crypto.randomUUID() });
        });
        ...
        mons.forEach((p) => {
            ...
            if (idx !== -1) mergedMons[idx] = { ...p, id: mergedMons[idx].id };
            else mergedMons.push({ ...p, id: crypto.randomUUID() });
        });
        ...
        items.forEach((i) => {
            ...
            if (idx !== -1) mergedItems[idx] = { ...i, id: mergedItems[idx].id };
            else mergedItems.push({ ...i, id: crypto.randomUUID() });
        });

        set({
            roomCustomTypes: mergedTypes,
            roomCustomAbilities: mergedAbs,
            roomCustomMoves: mergedMoves,
            roomCustomPokemon: mergedMons,
        ...
        try {
            ...
            Object.assign(roomSettings, {
                ...
                customAbilities: mergedAbs,
                customMoves: mergedMoves,
                customPokemon: mergedMons,
    ...
    addExtraCategory: () =>
        set((state) => {
            ...
            try {
                saveToOwlbear({ 'extra-skills-data': JSON.stringify(newCats) });
            } catch (e) {}
    ...
    updateExtraSkill: (catId, skillId, field, value) =>
        set((state) => {
    ...
    applyGeneratedBuild: (build) =>
        set((state) => {
            ...
            const newMoves: MoveData[] = build.moves.map((m) => {
                ...
                const properCat = catStr.startsWith('Phys')
                    ? 'Physical'
                    : catStr.startsWith('Spec')
                      ? 'Special'
                      : 'Status';
                ...
                return {
                    ...
                    id: crypto.randomUUID(),
                    active: false,
                    category: properCat as 'Physical' | 'Special' | 'Status',
                    accBonus: 0,
                    acc1: m.attr,
                    acc2: m.skill,
                    dmg1: m.dmgStat
            ...
            if (newExtraCats !== state.extraCategories)
                updatesToSave['extra-skills-data'] = JSON.stringify(newExtraCats);
